...
class AuthServiceImplTest {
  ...
  void login_shouldReturnResponse_whenCredentialsAreValid() {
    String ip = "1.2.3.4";
    LoginRequestDto request = LoginRequestDto.builder().username("luca").password("pass").build();

    UserEntity user = UserEntity.builder().id(1L).username("luca").password("hashed").build();
  ...
  void login_shouldRegisterFailedAndThrow_whenUserNotFound() {
    ...
    LoginRequestDto request =
        LoginRequestDto.builder().username("unknown").password("pass").build();
  ...
  void login_shouldRegisterFailedAndThrow_whenPasswordInvalid() {
    String ip = "1.2.3.4";
    LoginRequestDto request = LoginRequestDto.builder().username("luca").password("wrong").build();
  ...
  void login_shouldPropagateTooManyLoginAttemptsException_whenIpBlocked() {
    ...
    doThrow(new TooManyLoginAttemptsException(10L)).when(loginRateLimiter).checkAllowed(ip);
