...
class JwtTokenProviderTest {
    ...
    void validateAndGetUserId_shouldThrowInvalidJwtException_whenTokenIsTampered() {
        ...
        assertThrows(InvalidJwtException.class,
                () -> provider.validateAndGetUserId(tamperedToken));
    }
    ...
    void validateAndGetUserId_shouldThrowInvalidJwtException_whenSubjectIsNotNumeric() {
        ...
        assertThrows(InvalidJwtException.class,
                () -> provider.validateAndGetUserId(tokenWithNonNumericSubject));
    }

    @Test
    void validateAndGetUserId_shouldThrowInvalidJwtException_whenTokenIsExpired() throws InterruptedException {
        JwtTokenProvider provider = new JwtTokenProvider("test-secret", 1000);
        ...
        assertThrows(InvalidJwtException.class,
                () -> provider.validateAndGetUserId(token));
    }
}
