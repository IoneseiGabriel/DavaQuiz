...
class FilteringHelperTest {
  ...
  void setUp() {
    validFilters =
        new ConcurrentHashMap<>(Map.of("page", "1", "title", "Speed", "status", "DRAFT"));
    invalidFilters =
        new ConcurrentHashMap<>(Map.of("PAGE", "1", "title", "Speed", "status", "DRAFT"));
  ...
  void parseFiltersWithNullOrEmptyFiltersReturnsEmptyMap() {
    Assertions.assertEquals(
        Collections.emptyMap(), FilteringHelper.parseFilters(null, validAvailableFields));
    Assertions.assertEquals(
        Collections.emptyMap(),
        FilteringHelper.parseFilters(new ConcurrentHashMap<>(), validAvailableFields));
  }
    ...
    Assertions.assertAll(
        ...
        () ->
            assertThrows(
                ...
                () -> FilteringHelper.parseFilters(validFilters, null)),
        () ->
            assertThrows(
                IllegalArgumentException.class,
                () -> FilteringHelper.parseFilters(validFilters, set)));
  }
  ...
  void parseFiltersWithPageKeyReturnsFiltersWithNoPageKey() {
    ...
    // Act & Assert
    Assertions.assertEquals(
        expected, FilteringHelper.parseFilters(validFilters, validAvailableFields));
  }
