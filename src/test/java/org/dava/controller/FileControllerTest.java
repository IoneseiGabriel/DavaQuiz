...
class FileControllerTest {
    ...
    void uploadWithInvalidFileReturns400BadRequest() throws Exception {
        // Arrange
        String url = "/api/upload";
        MockMultipartFile noNameFile = new MockMultipartFile(
                "file",
                null,
                "image/webp",
                "image".getBytes());
        ...
        // Act & Assert
        mockMvc.perform(multipart(url).file(noNameFile))
                .andExpect(status().isInternalServerError());
    }
    ...
    void uploadWithFailedGeneratedUrlReturns500InternalServerError() throws Exception {
        ...
        // Act & Assert
        mockMvc.perform(multipart(url).file(FILE_WITH_INVALID_CONTENT_TYPE))
    ...
    void getByNameWithExistentFileReturnsOK200AndFileBytes() throws Exception {
        ...
        // Act & Assert
        mockMvc.perform(get(url + VALID_FILE.getOriginalFilename()))
                .andExpect(status().isOk())
    ...
    void getByNameWithNonexistentFileReturns404NotFound() throws Exception {
        mockMvc.perform(get("/api/images/missing.png"))
