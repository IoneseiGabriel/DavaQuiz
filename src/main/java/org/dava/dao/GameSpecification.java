...
/**
 * Specification class that provides reusable {@link Specification} instances for filtering {@link Game} entities.
 * <p>
 * Each private method in this class returns a {@code Specification<Game>} that can be combined
 * using {@link Specification#and(Specification)} to build dynamic query predicates at runtime.
 * </p>
 * <p>
 * This class is not intended to be instantiated and should be used by the service layer when constructing filtered queries.
 * </p>
 */
public class GameSpecification {
    ...
    /**
     * Based on the {@code filters} provided, a {@code Specification<Game>} query predicate is created.
     * If {@code filters} contain an invalid field name, an explicit {@link IllegalArgumentException} is thrown.
     *
    ...
    private static Specification<@NonNull Game> getSpecificationByKey(String key, Object value) {
        return switch (key) {
            ...
            default ->
                    throw new IllegalArgumentException(String.format("No field was found with the specified name '%s'", key));
        };
    }
}
