...
/**
 * In memory rate limiter that tracks failed login attempts per IP
 * and blocks further attempts for a configured time window.
 */
@Slf4j
@Component
public class LoginRateLimiter {
    ...
    /**
     * Verifies if the given IP address is currently allowed to attempt login.
     * If the IP is still blocked, a TooManyLoginAttemptsException is thrown.
     * If the block period has expired, the internal state for that IP is cleared.
     */
     ...
     * Old attempts outside the configured interval are discarded.
     * When the number of recent failed attempts reaches the configured threshold,
     * the IP is blocked for the configured block duration.
     */
    public void registerFailed(String ip) {
        Instant now = Instant.now();

        IpState state = stateByIp.computeIfAbsent(ip, k ->
                new IpState(new ArrayDeque<>(), null));
        ...
        if (attempts.size() >= maxFailedAttempts) {
            ...
            log.warn("IP {} blocked until {} after {} failed attempts in the last {} seconds",
                    ip, blockedUntil, attempts.size(), interval.getSeconds());
        }
    }

    /**
     * Internal state for a single IP address, holding timestamps
     * of failed attempts and the optional block expiration time.
     */
    @Data
    @AllArgsConstructor
